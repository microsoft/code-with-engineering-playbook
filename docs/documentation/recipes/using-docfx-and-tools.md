...
To get you started quickly from scratch, a zip-file is provided which can be used with some common folder, files and settings mentioned in the steps below. Copy the content from the [DocFxQuickStart folder](./DocFxQuickStart) folder to your own folder.
...
> 4. Fill in the default values in **infrastructure/variables.tf** and run the commands from [Step 4 - Deploy Azure resources from your local machine](deploy-docfx-azure-website.md#4-Running-the-commands-locally) to create the Azure Resources.
...
In the provided Quick Start folder a `.markdownlint.json` is included with the contents below. The [MD013 setting](https://github.com/markdownlint/markdownlint/blob/master/docs/RULES.md#md013---line-length) is set to false to prevent checking for maximum line length. You can modify this file to your likings to include or exclude certain tests.
...
After you completed the 6 steps, you should have a default website generated in the *_site* folder. But of course, you want this to be accessible for everyone. So, the next step is to create an Azure Website and have a process to automatically generate and deploy the contents to that website. That process is decribed in the recipe [Deploy the DocFx Documentation website to an Azure Website automatically](deploy-docfx-azure-website).
